"""
Lineage flowup/mmgp -> OpenMetadata (via REST API).

Os modelos flowup/mmgp criam as tabelas de destino dinamicamente (run_query),
então o dbt NÃO registra o lineage delas no manifest. Como o mapeamento é
1:1 pelo nome da tabela (raw_<x>.T -> mart_<x>.T), esta DAG cria as setas
diretamente na API do OpenMetadata, reaproveitando host/token do openmetadata.yaml.

Pré-requisito: as tabelas raw_* e mart_* já devem estar catalogadas no OM
...
# Pares (schema_origem, schema_destino) — mesma lógica do flowup/mmgp
SCHEMA_PAIRS = [
    ("raw_flowup", "mart_flowup"),
    ("raw_mmgp", "mart_mmgp"),
]
...
def criar_lineage(**context):
    ...
    for raw_schema, mart_schema in SCHEMA_PAIRS:
        # Só tabelas que existem em AMBOS os schemas
        rows = hook.get_records(
            ...
            """,
            parameters=(mart_schema, raw_schema),
        )
        tabelas = [row[0] for row in rows]
        logger.info("[%s -> %s] %d tabela(s) candidata(s)", raw_schema, mart_schema, len(tabelas))

        for t in tabelas:
            from_fqn = f"{OM_SERVICE}.{om_database}.{raw_schema}.{t}"
            to_fqn = f"{OM_SERVICE}.{om_database}.{mart_schema}.{t}"
    ...
    if total_ok == 0:
        raise ValueError(
            ...
            "se as tabelas raw_*/mart_* ja foram ingeridas no OpenMetadata."
