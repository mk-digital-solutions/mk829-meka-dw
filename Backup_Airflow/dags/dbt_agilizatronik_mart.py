...
with DAG(
    dag_id="dbt_agilizatronik_mart",
    start_date=datetime(2024, 1, 1),
    schedule="15 1 * * *",
    catchup=False,
    tags=["dbt", "agilizatronik", "mart"],
    params={
    ...
    render_config = RenderConfig(select=["agilizatronik_mart"], dbt_deps=False)

    transformacao = DbtTaskGroup(
        group_id="dbt_run_agilizatronik_mart",
        ...
        operator_args={
            ...
            "vars": {
                "stg_schema": "stg_agilizatronik",
                "modo_debug": "{{ params.modo_debug }}",
            },
            "select": "agilizatronik_mart",
